...
echo -e "${PURPLE}Running algorithm comparisons...${NORMAL}";
for EXPERIMENT in ./experiments_conf/algorithm/*.py; do
    echo -e "${PURPLE}Executing $EXPERIMENT...${NORMAL}";
    python3 $EXPERIMENT;
done;

echo -e "${PURPLE}Running constraint bounds comparisons...${NORMAL}";
for EXPERIMENT in ./experiments_conf/constraints/constraints_bounds/*.py; do
    echo -e "${PURPLE}Executing $EXPERIMENT...${NORMAL}";
    python3 $EXPERIMENT;
done;

echo -e "${PURPLE}Running number of constraints comparisons...${NORMAL}";
for EXPERIMENT in ./experiments_conf/constraints/number_of_constraints/*.py; do
    echo -e "${PURPLE}Executing $EXPERIMENT...${NORMAL}";
    python3 $EXPERIMENT;
done;

echo -e "${PURPLE}Running k* comparisons...${NORMAL}";
for EXPERIMENT in ./experiments_conf/k/*.py; do
    echo -e "${PURPLE}Executing $EXPERIMENT...${NORMAL}";
    python3 $EXPERIMENT;
done;

echo -e "${PURPLE}Running epsilon (maximum average deviation) comparisons...${NORMAL}";
for EXPERIMENT in ./experiments_conf/max_dev/*.py; do
    echo -e "${PURPLE}Executing $EXPERIMENT...${NORMAL}";
    python3 $EXPERIMENT;
done;

echo -e "${PURPLE}Running predicate type comparisons...${NORMAL}";
for EXPERIMENT in ./experiments_conf/predicate/*.py; do
    echo -e "${PURPLE}Executing $EXPERIMENT...${NORMAL}";
    python3 $EXPERIMENT;
done;

echo -e "${PURPLE}Running data size comparisons...${NORMAL}";
